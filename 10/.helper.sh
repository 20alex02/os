wrap_line()
{
    ...
    width=$((100 - $indent - 1)); # FIXME widen $COLUMNS in frag-eval
    c1="$1"; shift
    c2="$@"
    nl=
    while test -n "$c2"; do
...
grind()
{
    ...
    test -d /run/systemd/system && test -x /usr/bin/systemd-run && \
        timeout="systemd-run --user --scope --collect -pRuntimeMaxSec=10"
    ( $timeout true >/dev/null 2>&1 ) || timeout=''

    rm -f $vglog.*
    log $timeout $VALGRIND --error-exitcode=1 --log-file=$vglog.%p --leak-check=full "$@"
    err=$?
